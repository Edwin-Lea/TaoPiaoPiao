...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake((MaxX(self.imageView)+10), (MinY(self.imageView)+3), 245, 40)];
        ...
        [string setAttributes:@{NSForegroundColorAttributeName: COLOR_RGB(255, 77, 100)} range:NSMakeRange(strTitle.length, strPrice.length)];
...
- (UILabel *)timeAndCategoryLabel {
    if (!_timeAndCategoryLabel) {
        _timeAndCategoryLabel = [[UILabel alloc] initWithFrame:CGRectMake(MinX(self.titleLabel), (MaxX(self.titleLabel)+20), 245, 15)];
        _timeAndCategoryLabel.font = [UIFont systemFontOfSize:14.5];
...
- (UILabel *)addressLabel {
    if (!_addressLabel) {
        ...
        _addressLabel.font = [UIFont systemFontOfSize:14.5];
...
- (UILabel *)keyWordsView {
    if (!_keyWordsView) {
        _keyWordsView = [[UILabel alloc] initWithFrame:CGRectMake(0, 137, SCREEN_WIDTH, 47)];
        _keyWordsView.text = self.data.keyWords;
        _keyWordsView.textColor = COLOR_RGB(158, 154, 174);
        _keyWordsView.font = [UIFont systemFontOfSize:12.5];

        // 添加线
        UIView *underline = [[UIView alloc] initWithFrame:CGRectMake(MinX(self.addressLabel), 0, (SCREEN_WIDTH-MinX(self.addressLabel)), 1)];
